...
export function SearchWorkspace({ mode, showAll = false }: { mode: SearchMode; showAll?: boolean }) {
    ...
    return <div className="grid gap-6">
        ...
        {searched && <section><div className="mb-3 flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-ink">{title}</h2><span className="text-sm text-ink-muted">{total} found</span></div>{mode === "people" ? users.length ? <div className="grid gap-3">{users.map((user) => <Link key={user._id} href={`/users/${user._id}`}><Card className="flex items-center gap-3 p-4 transition-colors hover:border-accent"><span className="grid size-10 place-items-center rounded-full bg-accent-subtle text-accent"><UserRound className="size-5" /></span><span><strong className="block text-sm text-ink">{user.firstName} {user.lastName}</strong><span className="text-xs text-ink-muted">View profile</span></span></Card></Link>)}</div> : <EmptyState icon={<UserRound className="size-5" />} title="No people found">Try another name.</EmptyState> : events.length ? <div className="grid gap-3">{events.map((event) => <Link key={event._id} href={`/broadcasts/${event._id}`}><Card className="p-4 transition-colors hover:border-accent"><div className="flex items-start gap-3"><CalendarSearch className="mt-0.5 size-5 text-accent" /><span><strong className="block text-sm text-ink">{event.title ?? event.message}</strong><span className="mt-1 block text-xs text-ink-muted">{new Date(event.eventDate).toLocaleString()} · {event.postalCode}, {event.state}</span></span></div></Card></Link>)}</div> : <EmptyState icon={<CalendarSearch className="size-5" />} title="No events found">Try another place or keyword.</EmptyState>}{!fullResults && total > visibleLimit && <Link href={`/search/${mode}?q=${encodeURIComponent(query)}&all=1`} className="mt-4 inline-flex text-sm font-semibold text-accent hover:underline">View all {total} {mode}</Link>}</section>}
    </div>;
}
